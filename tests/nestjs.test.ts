...
import { setLanguageExtractionConfig } from '../src/nestjs/types';
...
describe('NestJS Integration', () => {
  ...
  describe('@I18nLanguageAware Decorator', () => {
    let dataSource: DataSource;
    ...
    describe('CQRS - extracting from payload', () => {
      it('should extract language from command and set on repos', async () => {
        const repo = getI18nRepository(Product, dataSource);
        ...
        class CreateProductHandler {
          ...
          async execute(_command: WithLanguage) {
            return this.productRepo.find({ where: { name: 'Portátil' } as any });
          }
        }
      ...
      it('should set language on multiple repos', async () => {
        const repo1 = getI18nRepository(Product, dataSource);
        const repo2 = getI18nRepository(Product, dataSource);
        ...
        class MultiRepoHandler {
          ...
          @I18nLanguageAware()
          async execute(_command: WithLanguage) {
            return [this.productRepo.getLanguage(), this.anotherRepo.getLanguage()];
          }
        }

        const handler = new MultiRepoHandler();
        const [lang1, lang2] = await handler.execute({ language: 'fr' });

        expect(lang1).toBe('fr');
        expect(lang2).toBe('fr');
      });

      it('should also set language on language service', async () => {
        const languageService = new I18nLanguageService();
      ...
      it('should use custom field name', async () => {
        ...
        const handler = new CustomFieldHandler();
        const result = await handler.execute({ lang: 'fr' });

        expect(result).toBe('fr');
      });

      it('should use default language when payload has none', async () => {
        const languageService = new I18nLanguageService();

        class DefaultHandler {
          languageService = languageService;

          @I18nLanguageAware({ defaultLanguage: 'de' })
          async execute(_command: WithLanguage) {
            return this.languageService.getLanguage();
          }
        }
    ...
    describe('Global config via I18nModule', () => {
      afterEach(() => {
        // Reset global config after each test
        setLanguageExtractionConfig({});
      });

      it('should use field from global config', async () => {
        // Set global config
        setLanguageExtractionConfig({ field: 'locale' });
        ...
        class GlobalConfigHandler {
          ...
          @I18nLanguageAware() // No options - uses global config
          async execute(_command: { locale?: string }) {
            return this.languageService.getLanguage();
          }
        }

        const handler = new GlobalConfigHandler();
        const result = await handler.execute({ locale: 'es' });

        expect(result).toBe('es');
      });

      it('should use headerField from global config', async () => {
        setLanguageExtractionConfig({ headerField: 'language' });
        ...
        class GlobalHeaderHandler {
          ...
          @I18nLanguageAware()
          async handleMessage(_payload: any, _context: any) {
            return this.productRepo.getLanguage();
          }
        }

        const handler = new GlobalHeaderHandler();
        const context = {
          getMessage: () => ({ headers: { 'language': 'fr' } }),
        };
      ...
      it('should allow decorator options to override global config', async () => {
        setLanguageExtractionConfig({ field: 'globalField' });
        ...
        class OverrideHandler {
          ...
          @I18nLanguageAware({ field: 'localField' }) // Override global
          async execute(_command: { localField?: string; globalField?: string }) {
            return this.languageService.getLanguage();
          }
        }

        const handler = new OverrideHandler();
        // Should use localField (decorator option), not globalField (global config)
        const result = await handler.execute({ localField: 'es', globalField: 'fr' });

        expect(result).toBe('es');
      });

      it('should work with I18nLanguageAwareHandler class decorator', async () => {
        setLanguageExtractionConfig({ field: 'lang' });
        ...
        const handler = new GlobalClassHandler();
        const result = await handler.execute({ lang: 'fr' });
    ...
    describe('Microservices - extracting from context headers', () => {
      it('should extract from Kafka headers when payload has no language', async () => {
        const repo = getI18nRepository(Product, dataSource);
        ...
        const handler = new KafkaHandler();
        const payload = { name: 'Test' };
        const context = {
          getMessage: () => ({ headers: { 'x-language': 'es' } }),
        };

        const result = await handler.handleMessage(payload, context);

        expect(result).toBe('es');
      });

      it('should handle Kafka Buffer headers', async () => {
        const repo = getI18nRepository(Product, dataSource);

        class KafkaBufferHandler {
          productRepo = repo;
        ...
        const handler = new KafkaBufferHandler();
        const payload = { name: 'Test' };
        const context = {
          getMessage: () => ({ headers: { 'x-language': Buffer.from('fr') } }),
        };
      ...
      it('should extract from RabbitMQ headers', async () => {
        const repo = getI18nRepository(Product, dataSource);
        ...
        const handler = new RabbitMQHandler();
        const payload = { name: 'Test' };
        const context = {
          getMessage: () => ({
      ...
      it('should extract from gRPC metadata', async () => {
        const repo = getI18nRepository(Product, dataSource);
        ...
        class GrpcHandler {
          ...
          @I18nLanguageAware()
          async handleMessage(_payload: any, _metadata: any) {
            return this.productRepo.getLanguage();
          }
        }
        ...
        const payload = { name: 'Test' };
        const metadata = {
          get: (key: string) => key === 'x-language' ? ['fr'] : [],
        };

        const result = await handler.handleMessage(payload, metadata);

        expect(result).toBe('fr');
      });

      it('should use custom header field', async () => {
        const repo = getI18nRepository(Product, dataSource);

        class CustomHeaderHandler {
          productRepo = repo;

          @I18nLanguageAware({ headerField: 'language' })
          async handleMessage(_payload: any, _context: any) {
            return this.productRepo.getLanguage();
          }
        }

        const handler = new CustomHeaderHandler();
        const payload = { name: 'Test' };
        const context = {
          getMessage: () => ({ headers: { 'language': 'es' } }),
        };
      ...
      it('should prefer payload over context', async () => {
        const repo = getI18nRepository(Product, dataSource);
  ...
  describe('@I18nLanguageAwareHandler Class Decorator', () => {
    ...
    it('should work with custom options', async () => {
      ...
      @I18nLanguageAwareHandler({ field: 'locale', defaultLanguage: 'en' })
      class LocaleHandler {
        languageService = languageService;
      ...
      const handler = new LocaleHandler();

      // With locale
      expect(await handler.execute({ locale: 'es' })).toBe('es');

      // Without locale (uses default)
      const handler2 = new LocaleHandler();
      expect(await handler2.execute({})).toBe('en');
    });

    it('should extract from context headers for microservices', async () => {
      const repo = getI18nRepository(Product, dataSource);
      ...
      class MessageHandler {
        ...
        async execute(_payload: any, _context: any) {
          return this.productRepo.getLanguage();
        }
      }
